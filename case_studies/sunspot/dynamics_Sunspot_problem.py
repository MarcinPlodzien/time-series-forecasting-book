...
"""
╔══════════════════════════════════════════════════════════════════════════╗
║   SUNSPOT MONTHLY DATA — Solar Cycle Dynamics                         ║
║   D_KY Estimation from Experimental Time Series                       ║
╚══════════════════════════════════════════════════════════════════════════╝

DATASET
───────
Monthly mean sunspot numbers (Wolf number), measuring solar magnetic activity.
The sunspot cycle has a period of ~11 years (≈132 months).This time series
exhibits QUASI-PERIODIC behavior with irregular amplitude modulations,
rather than fully developed chaos.

The dynamics are believed to arise from the solar dynamo — a magnetohydro-
dynamic (MHD) process where differential rotation and turbulent convection
generate and reverse the Sun's magnetic field. While the underlying equations
are high-dimensional PDEs, the emergent dynamics may lie on a low-dimensional
attractor.

EXPECTED RESULTS:
  - λ_1 ≈ 0 or slightly positive → quasi-periodic or weakly chaotic
  - D_KY ≈ 2-3 (low-dimensional dynamics)
  - The reconstructed attractor should show a noisy limit-cycle-like structure

The sunspot series is a classic test case where the answer is debated:
Farmer & Sidorowich (1987) estimated D_2 ≈ 2.3, while others argue the
apparent low dimensionality is an artifact of the ~11-year periodicity
and the true dynamics are high-dimensional or even stochastic.

ANALYSIS METHOD: Same Takens embedding pipeline as the other scripts.
See dynamics_SantaFe_laser_problem.py for mathematical details.
"""
...
DATA_FILE  = os.path.join(SCRIPT_DIR, "../../data/sunspot", "sunspot_monthly.txt")
DATA_NAME  = "Sunspot Monthly"
...
def plot_results(data, data_norm, tau, m, d_ky, lyap, lambda_1,
    ...
    fig.suptitle(
        ...
        f"$T_{{pred}} \\approx {T_pred:.0f}$ months",
    ...
    ax_ts.plot(t_months / 12.0, data, color='tab:orange', linewidth=0.7)
    ax_ts.set_xlabel("Year (relative)", fontsize=11)
    ax_ts.set_ylabel("Sunspot Number", fontsize=11)
    ...
    ax_raw.set_title(f"Raw Delay Plot ($\\tau = {tau}$ months)",
    ...
    acf = autocorrelation(data_norm, max_lag=min(400, len(data) // 4))
    ...
    fname = "../../figures/sunspot/sunspot_dky_analysis.png"
    plt.savefig(fname, dpi=300, bbox_inches='tight')
    print(f"  Saved: {fname}")

    # ═══ BONUS: Separate Power Spectrum figure ═══
    fig2, ax_psd = plt.subplots(figsize=(8, 4))
    freqs, psd = welch(data_norm, fs=1.0/12.0,
    ...
    fig2.tight_layout()
    plt.savefig("../../figures/sunspot/sunspot_psd.png", dpi=300, bbox_inches='tight')
    print(f"  Saved: ../figures/sunspot/sunspot_psd.png")
...
if __name__ == "__main__":
    ...
    with open("../../data/sunspot/sunspot_dky_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"  Exported: ../data/sunspot/sunspot_dky_results.json")
