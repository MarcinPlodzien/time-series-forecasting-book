...
"""
╔══════════════════════════════════════════════════════════════════════════╗
║   ENGLAND & WALES PRECIPITATION (EWP) — Monthly Totals                ║
║   Time Series Characterization & D_KY Estimation                      ║
╚══════════════════════════════════════════════════════════════════════════╝

DATASET
───────
The England & Wales Precipitation (EWP) series is the longest continuous
regional rainfall record in the world, maintained by the UK Met Office as
part of the HadUKP dataset.  Monthly precipitation totals (mm) from
January 1766 to the present (~3100 monthly values).

The series was constructed by Wigley & Jones (1987) and extended by
Gregory et al. (1991), Jones & Conway (1997), and Alexander & Jones (2001).
It averages over a network of rain gauge stations across England and Wales.

Source: https://www.metoffice.gov.uk/hadobs/hadukp/data/monthly/HadEWP_monthly_totals.txt
...
DATA_DIR   = os.path.join(SCRIPT_DIR, "..", "..", "data", "ewp")
FIG_DIR    = os.path.join(SCRIPT_DIR, "..", "..", "figures", "ewp")
DATA_NAME  = "England & Wales Precipitation (Monthly)"

EWP_URL = "https://www.metoffice.gov.uk/hadobs/hadukp/data/monthly/HadEWP_monthly_totals.txt"
...
# ══════════════════════════════════════════════════════════════════════════
def download_ewp():
    """Download EWP monthly data from the Met Office HadUKP archive."""
    raw_file = os.path.join(DATA_DIR, "ewp_monthly_raw.txt")

    if not os.path.exists(raw_file):
        print(f"  Downloading EWP data from Met Office...")
        os.makedirs(DATA_DIR, exist_ok=True)
        urllib.request.urlretrieve(EWP_URL, raw_file)
        print(f"  Saved: {raw_file}")
    else:
        print(f"  EWP data already downloaded: {raw_file}")

    return raw_file


def parse_ewp(raw_file):
    """Parse the Met Office EWP file format.

    Format: each row = Year  Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec  Annual
    Header lines start with text / are non-numeric.
    """
    years = []
    precip = []
    ...
    with open(raw_file, 'r') as f:
        for line in f:
            ...
            try:
                year = int(parts[0])
                if year < 1700 or year > 2030:
                    continue
                monthly = [float(parts[i]) for i in range(1, 13)]
                for month_idx, p in enumerate(monthly):
                    if p > -90:  # valid data (missing = -99.9)
                        years.append(year + (month_idx + 0.5) / 12.0)
                        precip.append(p)
            except (ValueError, IndexError):
                continue

    return np.array(years), np.array(precip)
...
# 7. PLOTTING
# ══════════════════════════════════════════════════════════════════════════
def plot_results(years, precip, data_norm, tau, m, d_ky, lyap,
                 lambda_1, t_div, avg_div, valid_div, fnn_fracs):
    """
    2×3 comprehensive predictability characterization figure:
      Row 1: (0) Precipitation series  (1) Delay embedding  (2) ACF
    ...
    # ═══ (0,0): Precipitation time series ═══
    ax_ts = fig.add_subplot(gs[0, 0])
    ax_ts.plot(years, precip, color='steelblue', linewidth=0.3, alpha=0.5)
    # 12-month running mean
    if len(precip) > 12:
        kernel = np.ones(12) / 12.0
        smooth = np.convolve(precip, kernel, mode='valid')
        years_smooth = years[5:5+len(smooth)]
        ax_ts.plot(years_smooth, smooth, color='darkblue', linewidth=1.0,
                   label='12-mo mean')
    ax_ts.set_xlabel("Year", fontsize=11)
    ax_ts.set_ylabel("Precipitation (mm/month)", fontsize=11)
    ax_ts.set_title("England & Wales Precipitation", fontsize=12, fontweight='bold')
    ...
    if m >= 3:
        ...
        ax_emb.set_ylabel(f"$P(t + {2*tau})$", fontsize=11)
    else:
        ax_emb.scatter(X_embed[::2, 0], X_embed[::2, 1], s=0.5, alpha=0.3,
                      color='steelblue')
        ax_emb.set_ylabel(f"$P(t + {tau})$", fontsize=11)
    ax_emb.set_xlabel(f"$P(t)$", fontsize=11)
    ax_emb.set_title(
    ...
    fname = os.path.join(FIG_DIR, "ewp_dky_analysis.png")
...
if __name__ == "__main__":
    ...
    # ── Download & parse ──
    raw_file = download_ewp()
    years, precip = parse_ewp(raw_file)
    print(f"  Loaded {len(precip)} monthly values ({years[0]:.0f}--{years[-1]:.0f})")
    print(f"  Mean: {np.mean(precip):.1f} mm, Std: {np.std(precip):.1f} mm")

    # ── Save processed data ──
    os.makedirs(DATA_DIR, exist_ok=True)
    np.savetxt(os.path.join(DATA_DIR, "ewp_monthly_precip.txt"), precip)
    np.savetxt(os.path.join(DATA_DIR, "ewp_monthly_years.txt"), years)

    # ── Work with anomalies (remove seasonal cycle) ──
    month_indices = np.round((years % 1) * 12).astype(int) % 12
    climatology = np.zeros(12)
    for m_idx in range(12):
        mask = month_indices == m_idx
        climatology[m_idx] = np.mean(precip[mask])
    anomalies = precip - climatology[month_indices]
    print(f"  Monthly climatology range: {climatology.min():.0f}--{climatology.max():.0f} mm")
    print(f"  Anomaly std: {np.std(anomalies):.2f} mm")
    ...
    results_file = os.path.join(DATA_DIR, "ewp_dky_results.json")
    with open(results_file, "w") as f:
        json.dump(results, f, indent=2)
    print(f"  Exported: {results_file}")

    # ── Plot ──
    plot_results(years, precip, data_norm, tau, m_opt, d_ky, lyap,
                 lambda_1, t_div, avg_div, valid_div, fnn_fracs)
